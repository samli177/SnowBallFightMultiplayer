  ...
  (class object%
    ...
    (define (listen-for-data)
      (define (loop)
        ...
        (if (eq? (string->symbol (car remote-word-list)) 'sync) (begin (semaphore-wait) (set! sync #t) (semaphore-post))
    ...
    (define (send-thread)
      (define (loop)
        (if (and (not (eq? change-check *object-list*)) sync)
            (begin (send-string (make-message *object-list*)) (set! change-check *object-list*) (begin (semaphore-wait) (set! sync #f) (semaphore-post))))
        (sleep .01)
        (loop))
